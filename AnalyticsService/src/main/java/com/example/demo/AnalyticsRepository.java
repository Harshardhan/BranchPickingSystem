...
public interface AnalyticsRepository extends JpaRepository<Analytics, Long> {
    ...
    @Query("SELECT AVG(a.deliveryDurationMinutes) " +
           "FROM Analytics a " +
           "WHERE a.deliveryStatus = com.example.demo.DeliveryStatus.DELIVERED")
    ...
    List<Analytics> findDelayedDeliveries(@Param("threshold") Long threshold);

    // 4️⃣ Delivery performance by product
    @Query("SELECT a.productId, AVG(a.deliveryDurationMinutes) " +
           "FROM Analytics a " +
           "WHERE a.deliveryStatus = com.example.demo.DeliveryStatus.DELIVERED " +
           "GROUP BY a.productId")
    List<Object[]> getAverageDeliveryTimeByProduct();

    // 5️⃣ Delivery performance by region
    @Query("SELECT a.region, AVG(a.deliveryDurationMinutes) " +
           "FROM Analytics a " +
           "WHERE a.deliveryStatus = com.example.demo.DeliveryStatus.DELIVERED " +
           "GROUP BY a.region")
    List<Object[]> getAverageDeliveryTimeByRegion();

    // 6️⃣ Total orders vs Delivered count
    @Query("SELECT COUNT(a) FROM Analytics a WHERE a.deliveryStatus = com.example.demo.DeliveryStatus.DELIVERED")
